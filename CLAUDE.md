...
- Run every local lane before pushing: `cargo fmt --all --check`, the three
  ...
  suite, the tls+prometheus suite, `RUSTDOCFLAGS="-D warnings" cargo doc`,
  and the rightsize container suite. Repeat timing-sensitive new tests ten
  times. CI is the end gate; dispatch it on the branch and wait for green.
- Run `cargo semver-checks --baseline-version <last release> --release-type
  minor` before a minor release; only intentional, changelog-listed breaks
  may remain.
- Reads are TTL-blind: only writes take a per-entry TTL. No read method
  accepts a TTL.

...
- **Per-entry TTL**: `Cache::insert_with_ttl` and `Cache::insert_many_with_ttl`
  ...
  `.ttl(..)` default in either direction and working on a cache configured
  with no default. The per-entry deadline is stamped as the record's
  absolute `expires_at_ms` and replicates exactly as a default-TTL stamp
  does, so the entry expires at the same instant on every node and carries
  the same can't-resurrect guarantee. Reads stay out of it: `get_or_load`
  fills take the cache default. The `Shard` API gains the same two methods.

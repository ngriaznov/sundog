    ...
    pub(crate) async fn close_spill_checkpointed(&self) {
        ...
        if let Some(tier) = self.engine.spill().cloned() {
            ...
            if tier.warm_reopen_value() {
                ...
                let newly_written = tokio::task::spawn_blocking(move || {
                ...
                // below.
                let engine = Arc::clone(&self.engine);
                let survivors = tokio::task::spawn_blocking(move || {
...
mod tests {
    ...
    async fn close_spill_checkpoint_drops_a_key_deleted_between_capture_and_disk_write() {
        ...
        let newly_written = tier.checkpoint_flush(sink.as_ref(), resident);
        assert_eq!(
            newly_written.len(),
            1,
        ...
        let survivors = shard1.engine.finalize_checkpoint_writes(newly_written, now);
        assert!(
            survivors.is_empty(),

...
use super::{InboundMsg, RequestHandler};
use crate::error::CodecError;
use crate::node::NodeId;
use crate::wire::{self, MAX_FRAME, Msg};

pub(super) type PeerFramed = Framed<TcpStream, LengthDelimitedCodec>;

pub(super) fn new_framed(stream: TcpStream) -> PeerFramed {
...
/// Dials `addr` on a fresh connection and completes the `Hello` handshake,
/// for the one-shot request/response paths (state transfer, anti-entropy).
...
pub(super) async fn run_peer_writer(
    ...
    'reconnect: loop {
        let mut framed = tokio::select! {
            ...
            connected = connect_with_hello(addr, local_node, incarnation, &cancel) => match connected {
                Some(framed) => framed,
...
async fn connect_with_hello(
    ...
    loop {
        if let Ok(framed) = dial_with_hello(addr, node, incarnation).await {
            return Some(framed);
        }
        tokio::select! {
            biased;
    ...
    loop {
        tokio::select! {
            ...
            accepted = listener.accept() => {
                ...
                tokio::spawn(handle_accepted(stream, inbound_tx.clone(), Arc::clone(&handler)));
            }
        }
    }
}

/// Serves one accepted connection: requires `Hello` first, then dispatches
/// each subsequent message. `Invalidate`/`Replicate` keep the connection
/// open and forward to `inbound_tx` (the persistent mesh-link case); a
/// request message (`StRequest`/`AeDigest`/`AePull`) is served once and the
/// connection is then closed, per plan "own streams" for request/response.
...
mod tests {
    ...
    async fn codec_roundtrips_a_message_through_a_real_socket_pair() {
        ...
        send_msg(&mut client, &sent).await.expect("send");
        let got = recv_msg(&mut server)
            .await
